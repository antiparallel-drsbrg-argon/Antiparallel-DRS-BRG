...
*/
static clock_t run(uint32_t outlen, char *pwd, size_t pwdlen, char *salt, uint32_t t_cost,
...
int main(int argc, char *argv[]) {
	...
	salt = "12345678";
	FILE *fp;
	fp = fopen("Argon_no_anti.txt", "w");
	for (i = 0; i < 1000; i++) {
		time = run(outlen, pwd, pwdlen, salt, t_cost, m_cost, lanes, threads, type,
